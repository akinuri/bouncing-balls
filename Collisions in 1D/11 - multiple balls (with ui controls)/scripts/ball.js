var Ball = function (r, px, py, vx, vy, fillStyle) {
    ...
    this.style            = {};
    this.style.color      = fillStyle || "black";
    this.style.xrayFill   = color(this.style.color).alpha(.2).rgba();
    this.style.xrayStroke = color(this.style.color).alpha(.5).sat(-50).lum(-50).rgba();
    this.style.wfStroke   = color(this.style.color).sat(-30).lum(-20).rgba();
};
...
Ball.prototype.move = function () {
    ...
    tempVel.multiply(globals.speed);
...
Ball.prototype.draw = function () {
    ...
    if (globals.wireframe) {
        context.strokeStyle = this.style.wfStroke;
        context.stroke();
    }
    else if (globals.xray) {
        context.fillStyle = this.style.xrayFill;
        context.fill();
        context.strokeStyle = this.style.xrayStroke;
        context.stroke();
    } else {
...
function spawnBalls(count, radius) {
    ...
    for (var i = 0; i < cells.length; i++) {
        balls.push(new Ball(radius, random(cells[i].x1, cells[i].x2), 25, random(0, 4), 0, color.random().hex()));
    }
    return balls;
}
